import {CanvasNode, FrameNode, framer} from "framer-plugin";
...
export interface saveAltTextProps {
  nodeId: string;
  altText: string;
  isAllowed: boolean;
}

export async function saveAltText({nodeId, altText, isAllowed}: saveAltTextProps) {
  try {
    const node = await framer.getNode(nodeId);
    const imgNode = node as FrameNode;
    if (isAllowed) {
      imgNode.setAttributes({
        backgroundImage: imgNode.backgroundImage?.cloneWithAttributes({
          altText
        })
      });
    }
  } catch (error) {
    console.error('Error saving the Alt Text :', error);
  }
}