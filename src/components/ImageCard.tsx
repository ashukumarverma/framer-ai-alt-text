...
import {saveAltText} from "../utils/framerHelpers.ts";
...
export function ImageCard({image, onGenerate, onAltTextChange,}: ImageCardProps) {
  const handleTextareaChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    onAltTextChange(image.id, e.target.value);
  };

  const isAllowed = useIsAllowedTo("setAttributes");
  useEffect(() => {
    saveAltText({nodeId: image.id, altText: image.alt, isAllowed});
  }, [image.alt]);
